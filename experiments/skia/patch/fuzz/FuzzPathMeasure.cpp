...
DEF_FUZZ(PathMeasure, fuzz) {
    ...
    FUZZ_ENDPOINT(SkRect bounds = path.getBounds();
    ...
    SkPoint position;
    SkVector tangent;
    FUZZ_ENDPOINT(ignoreResult(measure.getPosTan(distance[0], &position, &tangent));)
    SkPath dst;
    FUZZ_ENDPOINT(ignoreResult(measure.getSegment(distance[1], distance[2], &dst, (bits >> 1) & 1));
