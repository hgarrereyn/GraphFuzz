...
void AddASCIIText(ParagraphBuilder* builder,Fuzz* fuzz) {
    ...
    FUZZ_ENDPOINT(builder->addText(text, text_length);)
}
// Add random bytes to the paragraph.
void AddUnicodeText(ParagraphBuilder* builder,Fuzz* fuzz) {
    char16_t text[MAX_TEXT_LENGTH];
    const auto text_length = RandomText(text, fuzz);
    FUZZ_ENDPOINT(builder->addText(std::u16string(text, text_length));)
}
...
void AddZalgoText(ParagraphBuilder* builder, Fuzz* fuzz) {
    ...
    FUZZ_ENDPOINT(builder->addText(result);)
}
...
void AddStyle(ParagraphBuilder* builder, Fuzz*  fuzz) {
    ...
    TextStyle ts;
    FUZZ_ENDPOINT(ts.setFontFamilies({SkString("Roboto")});)
    ...
    FUZZ_ENDPOINT(builder->pushStyle(ts);)
}
void RemoveStyle(ParagraphBuilder* builder, Fuzz*  fuzz) {
    bool pop;
    fuzz->next(&pop);
    FUZZ_ENDPOINT(if (pop) {
        builder->pop();
    })
}
...
ParagraphStyle BuildParagraphStyle(Fuzz* fuzz) {
    ...
    bool hinting;
    fuzz->next(&hinting);
    FUZZ_ENDPOINT(if (hinting) {
        ps.turnHintingOff();
    })
    StrutStyle ss;
    // TODO(westont): Fuzz this object.
    FUZZ_ENDPOINT(ps.setStrutStyle(ss);)
    TextDirection td;
    fuzz->nextEnum(&td, TextDirection::kRtl);
    FUZZ_ENDPOINT(ps.setTextDirection(td);)
    TextAlign ta;
    fuzz->nextEnum(&ta, TextAlign::kEnd);
    FUZZ_ENDPOINT(ps.setTextAlign(ta);)
    size_t ml;
    fuzz->next(&ml);
    FUZZ_ENDPOINT(ps.setMaxLines(ml);)
    // TODO(westont): Randomize with other values and no value.
    FUZZ_ENDPOINT(ps.setEllipsis(u"\u2026");)
    SkScalar h;
    fuzz->next(&h);
    FUZZ_ENDPOINT(ps.setHeight(h);)
    ...
    FUZZ_ENDPOINT(ps.setTextHeightBehavior(thb);)
...
DEF_FUZZ(SkParagraph, fuzz) {
    ...
    FUZZ_ENDPOINT(ParagraphBuilderImpl builder(paragraph_style, fontCollection);)

    uint8_t iterations;
    fuzz->nextRange(&iterations, 1, MAX_TEXT_ADDITIONS);
    for (int i = 0; i < iterations; i++) {
        FUZZ_ENDPOINT(AddStyleAndText(&builder, fuzz);)
    }
    // TODO(westont): Figure out if we can get more converage by having fontsFound, current
    // they're not.
    // if (!fontCollection->fontsFound()) return;

    FUZZ_ENDPOINT(builder.pop();
    ...
    FUZZ_ENDPOINT(SkCanvas canvas(bm);)
    uint8_t layout_width;
    fuzz->next(&layout_width);
    FUZZ_ENDPOINT(paragraph->layout(layout_width);
