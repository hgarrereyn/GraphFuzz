...
static sk_sp<SkColorFilter> make_fuzz_colorfilter(Fuzz* fuzz, int depth) {
    ...
    switch (colorFilterType) {
        ...
        case 1: {
            ...
            return FUZZ_ENDPOINT(SkColorFilters::Blend(color, mode);)
        }
        case 2: {
            ...
            return FUZZ_ENDPOINT(outer->makeComposed(std::move(inner));)
        }
        case 3: {
            float array[20];
            fuzz->nextN(array, SK_ARRAY_COUNT(array));
            return FUZZ_ENDPOINT(SkColorFilters::Matrix(array);)
        }
        case 4: {
            SkColor mul, add;
            fuzz->next(&mul, &add);
            return FUZZ_ENDPOINT(SkColorMatrixFilter::MakeLightingFilter(mul, add);)
        }
        case 5: {
            bool grayscale;
            int invertStyle;
            ...
            return FUZZ_ENDPOINT(SkHighContrastFilter::Make(SkHighContrastConfig(
                    grayscale, SkHighContrastConfig::InvertStyle(invertStyle), contrast));)
        }
        case 6:
            return FUZZ_ENDPOINT(SkLumaColorFilter::Make();)
        case 7: {
            uint8_t table[256];
            fuzz->nextN(table, SK_ARRAY_COUNT(table));
            return FUZZ_ENDPOINT(SkTableColorFilter::Make(table);)
        }
        case 8: {
            uint8_t tableA[256];
            uint8_t tableR[256];
            ...
            return FUZZ_ENDPOINT(SkTableColorFilter::MakeARGB(tableA, tableR, tableG, tableB);)
        }
        default:
            SkASSERT(false);
            break;
...
static sk_sp<SkShader> make_fuzz_shader(Fuzz* fuzz, int depth) {
    ...
    switch (shaderType) {
        case 0:
            return nullptr;
        case 1:
            return FUZZ_ENDPOINT(SkShaders::Empty();)
        case 2:
            fuzz->next(&color);
            return FUZZ_ENDPOINT(SkShaders::Color(color);)
        case 3:
            ...
            return FUZZ_ENDPOINT(img->makeShader(tmX, tmY, SkSamplingOptions(), useMatrix ? &matrix : nullptr);)
        case 5:
            shader1 = make_fuzz_shader(fuzz, depth - 1);  // limit recursion.
            FuzzNiceMatrix(fuzz, &matrix);
            return FUZZ_ENDPOINT(shader1 ? shader1->makeWithLocalMatrix(matrix) : nullptr;)
        case 6:
            shader1 = make_fuzz_shader(fuzz, depth - 1);  // limit recursion.
            colorFilter = make_fuzz_colorfilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(shader1 ? shader1->makeWithColorFilter(std::move(colorFilter)) : nullptr;)
        case 7:
            shader1 = make_fuzz_shader(fuzz, depth - 1);  // limit recursion.
            shader2 = make_fuzz_shader(fuzz, depth - 1);
            fuzz->nextEnum(&blendMode, SkBlendMode::kLastMode);
            return FUZZ_ENDPOINT(SkShaders::Blend(blendMode, std::move(shader1), std::move(shader2));)
        case 8: {
            ...
            return FUZZ_ENDPOINT(pic->makeShader(tmX, tmY, SkFilterMode::kNearest,
        ...
        case 10: {
            ...
            return FUZZ_ENDPOINT(SkGradientShader::MakeLinear(pts, colors, usePos ? pos : nullptr, colorCount,
        ...
        case 11: {
            ...
            return FUZZ_ENDPOINT(SkGradientShader::MakeRadial(center, radius, colors, usePos ? pos : nullptr,
        ...
        case 12: {
            ...
            return FUZZ_ENDPOINT(SkGradientShader::MakeTwoPointConical(start, startRadius, end, endRadius, colors,
        ...
        case 13: {
            ...
            return FUZZ_ENDPOINT(SkGradientShader::MakeSweep(cx, cy, colors, usePos ? pos : nullptr, colorCount,
...
static sk_sp<SkPathEffect> make_fuzz_patheffect(Fuzz* fuzz, int depth) {
    ...
    switch (pathEffectType) {
        ...
        case 1: {
            ...
            return FUZZ_ENDPOINT(SkPathEffect::MakeSum(std::move(first), std::move(second));)
        }
        case 2: {
            sk_sp<SkPathEffect> first = make_fuzz_patheffect(fuzz, depth - 1);
            sk_sp<SkPathEffect> second = make_fuzz_patheffect(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkPathEffect::MakeCompose(std::move(first), std::move(second));)
        }
        case 3: {
            SkPath path;
            ...
            return FUZZ_ENDPOINT(SkPath1DPathEffect::Make(path, advance, phase, style);)
        }
        case 4: {
            SkScalar width;
            SkMatrix matrix;
            fuzz->next(&width);
            FuzzNiceMatrix(fuzz, &matrix);
            return FUZZ_ENDPOINT(SkLine2DPathEffect::Make(width, matrix);)
        }
        case 5: {
            SkPath path;
            FuzzNicePath(fuzz, &path, 20);
            SkMatrix matrix;
            FuzzNiceMatrix(fuzz, &matrix);
            return FUZZ_ENDPOINT(SkPath2DPathEffect::Make(matrix, path);)
        }
        case 6: {
            SkScalar radius;
            fuzz->next(&radius);
            return FUZZ_ENDPOINT(SkCornerPathEffect::Make(radius);)
        }
        case 7: {
            SkScalar phase;
            fuzz->next(&phase);
            ...
            return FUZZ_ENDPOINT(SkDashPathEffect::Make(intervals, count, phase);)
        }
        case 8: {
            SkScalar segLength, dev;
            uint32_t seed;
            fuzz->next(&segLength, &dev, &seed);
            return FUZZ_ENDPOINT(SkDiscretePathEffect::Make(segLength, dev, seed);)
        }
        default:
            SkASSERT(false);
            return nullptr;
...
static sk_sp<SkMaskFilter> make_fuzz_maskfilter(Fuzz* fuzz) {
    ...
    switch (maskfilterType) {
        ...
        case 1: {
            ...
            return FUZZ_ENDPOINT(SkMaskFilter::MakeBlur(blurStyle, sigma, respectCTM);)
        }
        default:
            SkASSERT(false);
            return nullptr;
...
static sk_sp<SkTypeface> make_fuzz_typeface(Fuzz* fuzz) {
    ...
    FUZZ_ENDPOINT(auto fontMugger = SkFontMgr::RefDefault();
    SkASSERT(fontMugger);
    ...
    return FUZZ_ENDPOINT(sk_sp<SkTypeface>(family->createTypeface(j));)
}
...
static sk_sp<SkImageFilter> make_fuzz_lighting_imagefilter(Fuzz* fuzz, int depth) {
    ...
    switch (imageFilterType) {
        case 1:
            ...
            return FUZZ_ENDPOINT(SkImageFilters::DistantLitDiffuse(p, lightColor, surfaceScale, k,
                                                     std::move(input),
                                                     useCropRect ? &cropRect : nullptr);)
        case 2:
            fuzz->next(&p, &lightColor, &surfaceScale, &k);
            input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::PointLitDiffuse(p, lightColor, surfaceScale, k,
                                                   std::move(input),
                                                   useCropRect ? &cropRect : nullptr);)
        case 3:
            fuzz->next(&p, &q, &specularExponent, &cutoffAngle, &lightColor, &surfaceScale, &k);
            input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::SpotLitDiffuse(
        ...
        case 4:
            ...
            input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::DistantLitSpecular(p, lightColor, surfaceScale, k,
                                                      shininess, std::move(input),
                                                      useCropRect ? &cropRect : nullptr);)
        case 5:
            fuzz->next(&p, &lightColor, &surfaceScale, &k, &shininess);
            input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::PointLitSpecular(p, lightColor, surfaceScale, k,
                                                    shininess, std::move(input),
                                                    useCropRect ? &cropRect : nullptr);)
        case 6:
            fuzz->next(&p, &q, &specularExponent, &cutoffAngle, &lightColor, &surfaceScale, &k,
                       &shininess);
            input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::SpotLitSpecular(
...
static sk_sp<SkImageFilter> make_fuzz_imageFilter(Fuzz* fuzz, int depth) {
    ...
    switch (imageFilterType) {
        ...
        case 1: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Blur(sigmaX, sigmaY, std::move(input),
                                        useCropRect ? &cropRect : nullptr);)
        }
        case 2: {
            SkMatrix matrix;
            ...
            FUZZ_ENDPOINT(auto sampling = SkSamplingOptions(quality);)
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::MatrixTransform(matrix, sampling, std::move(input));)
        }
        case 3: {
            SkRegion region;
            ...
            return FUZZ_ENDPOINT(SkImageFilters::AlphaThreshold(region, innerMin, outerMax, std::move(input),
                                                  useCropRect ? &cropRect : nullptr);)
        }
        case 4: {
            float k1, k2, k3, k4;
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Arithmetic(k1, k2, k3, k4, enforcePMColor,
        ...
        case 5: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::ColorFilter(std::move(cf), std::move(input),
                                               useCropRect ? &cropRect : nullptr);)
        }
        case 6: {
            sk_sp<SkImageFilter> ifo = make_fuzz_imageFilter(fuzz, depth - 1);
            sk_sp<SkImageFilter> ifi = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Compose(std::move(ifo), std::move(ifi));)
        }
        case 7: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::DisplacementMap(xChannelSelector, yChannelSelector, scale,
        ...
        case 9:
            return FUZZ_ENDPOINT(SkImageFilters::Image(make_fuzz_image(fuzz));)
        case 10: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Image(std::move(image), srcRect, dstRect,
        ...
        case 12: {
            ...
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Magnifier(srcRect, inset, std::move(input),
                                             useCropRect ? &cropRect : nullptr);)
        }
        case 13: {
            ...
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::MatrixConvolution(
        ...
        case 14: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Merge(std::move(first), std::move(second),
                                         useCropRect ? &cropRect : nullptr);)
        }
        case 15: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Merge(ifs, count, useCropRect ? &cropRect : nullptr);)
        }
        case 16: {
            int rx, ry;
            fuzz->next(&rx, &ry);
            ...
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Dilate(rx, ry, std::move(input),
                                          useCropRect ? &cropRect : nullptr);)
        }
        case 17: {
            int rx, ry;
            ...
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Erode(rx, ry, std::move(input),
                                         useCropRect ? &cropRect : nullptr);)
        }
        case 18: {
            SkScalar dx, dy;
            ...
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Offset(dx, dy, std::move(input),
                                          useCropRect ? &cropRect : nullptr);)
        }
        case 19: {
            SkPaint paint;
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Paint(paint, useCropRect ? &cropRect : nullptr);)
        }
        case 20: {
            sk_sp<SkPicture> picture = make_fuzz_picture(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Picture(std::move(picture));)
        }
        case 21: {
            SkRect cropRect;
            fuzz->next(&cropRect);
            sk_sp<SkPicture> picture = make_fuzz_picture(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Picture(std::move(picture), cropRect);)
        }
        case 22: {
            SkRect src, dst;
            fuzz->next(&src, &dst);
            sk_sp<SkImageFilter> input = make_fuzz_imageFilter(fuzz, depth - 1);
            return FUZZ_ENDPOINT(SkImageFilters::Tile(src, dst, std::move(input));)
        }
        case 23: {
            SkBlendMode blendMode;
            bool useCropRect;
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Blend(blendMode, std::move(bg), std::move(fg),
                                         useCropRect ? &cropRect : nullptr);)
        }
        case 24: {
            ...
            return FUZZ_ENDPOINT(SkImageFilters::Shader(std::move(shader), useCropRect ? &cropRect : nullptr);)
        }
        default:
            SkASSERT(false);
            return nullptr;
...
static sk_sp<SkImage> make_fuzz_image(Fuzz* fuzz) {
    ...
    FUZZ_ENDPOINT(SkAutoTMalloc<SkPMColor> data(w * h);
    SkPixmap pixmap(SkImageInfo::MakeN32Premul(w, h), data.get(), w * sizeof(SkPMColor));)
    int n = w * h;
    for (int i = 0; i < n; ++i) {
        SkColor c;
        fuzz->next(&c);
        FUZZ_ENDPOINT(data[i] = SkPreMultiplyColor(c);)
    }
    FUZZ_ENDPOINT((void)data.release();)
    return FUZZ_ENDPOINT(SkImage::MakeFromRaster(pixmap, [](const void* p, void*) { sk_free((void*)p); },
                                   nullptr);)
}
...
static SkFont fuzz_font(Fuzz* fuzz) {
    SkFont font;
    FUZZ_ENDPOINT(font.setTypeface(           make_fuzz_typeface(fuzz));
...
static SkTDArray<uint8_t> make_fuzz_text(Fuzz* fuzz, const SkFont& font, SkTextEncoding encoding) {
    ...
    if (SkTextEncoding::kGlyphID == encoding) {
        int glyphRange = FUZZ_ENDPOINT(font.getTypefaceOrDefault()->countGlyphs();)
        if (glyphRange == 0) {
...
static sk_sp<SkTextBlob> make_fuzz_textblob(Fuzz* fuzz) {
    ...
    while (runCount-- > 0) {
        ...
        FUZZ_ENDPOINT(font.setEdging(make_fuzz_t<bool>(fuzz) ? SkFont::Edging::kAlias : SkFont::Edging::kAntiAlias);)
        SkTDArray<uint8_t> text = make_fuzz_text(fuzz, font, encoding);
        int glyphCount = FUZZ_ENDPOINT(font.countText(text.begin(), SkToSizeT(text.count()), encoding);)
        SkASSERT(glyphCount <= kMaxGlyphCount);
        SkScalar x, y;
        const SkTextBlobBuilder::RunBuffer* buffer;
        uint8_t runType;
        fuzz->nextRange(&runType, (uint8_t)0, (uint8_t)2);
        FUZZ_ENDPOINT(const void* textPtr = text.begin();
        size_t textLen =  SkToSizeT(text.count());)
        switch (runType) {
            case 0:
                fuzz->next(&x, &y);
                // TODO: Test other variations of this.
                FUZZ_ENDPOINT(buffer = &textBlobBuilder.allocRun(font, glyphCount, x, y);
                (void)font.textToGlyphs(textPtr, textLen, encoding, buffer->glyphs, glyphCount);)
                break;
            case 1:
                fuzz->next(&y);
                // TODO: Test other variations of this.
                FUZZ_ENDPOINT(buffer = &textBlobBuilder.allocRunPosH(font, glyphCount, y);
                (void)font.textToGlyphs(textPtr, textLen, encoding, buffer->glyphs, glyphCount);
                fuzz->nextN(buffer->pos, glyphCount);)
                break;
            case 2:
                // TODO: Test other variations of this.
                FUZZ_ENDPOINT(buffer = &textBlobBuilder.allocRunPos(font, glyphCount);
                (void)font.textToGlyphs(textPtr, textLen, encoding, buffer->glyphs, glyphCount);
    ...
    return FUZZ_ENDPOINT(textBlobBuilder.make();)
}
...
static void fuzz_canvas(Fuzz* fuzz, SkCanvas* canvas, int depth = 9) {
    ...
    FUZZ_ENDPOINT(SkAutoCanvasRestore autoCanvasRestore(canvas, false);)
    unsigned N;
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        switch (drawCommand) {
            case 0:
                FUZZ_ENDPOINT(canvas->flush();)
                break;
            case 1:
                FUZZ_ENDPOINT(canvas->save();)
                break;
            case 2: {
                SkRect bounds;
                fuzz->next(&bounds);
                fuzz_paint(fuzz, &paint, depth - 1);
                FUZZ_ENDPOINT(canvas->saveLayer(&bounds, &paint);)
                break;
            }
            case 3: {
                SkRect bounds;
                fuzz->next(&bounds);
                FUZZ_ENDPOINT(canvas->saveLayer(&bounds, nullptr);)
                break;
            }
            case 4:
                fuzz_paint(fuzz, &paint, depth - 1);
                FUZZ_ENDPOINT(canvas->saveLayer(nullptr, &paint);)
                break;
            case 5:
                FUZZ_ENDPOINT(canvas->saveLayer(nullptr, nullptr);)
                break;
            case 6: {
                uint8_t alpha;
                fuzz->next(&alpha);
                FUZZ_ENDPOINT(canvas->saveLayerAlpha(nullptr, (U8CPU)alpha);)
                break;
            }
            case 7: {
                SkRect bounds;
                ...
                FUZZ_ENDPOINT(canvas->saveLayerAlpha(&bounds, (U8CPU)alpha);)
                break;
            }
            case 8: {
                ...
                if (make_fuzz_t<bool>(fuzz)) {
                    fuzz->next(&bounds);
                    FUZZ_ENDPOINT(saveLayerRec.fBounds = &bounds;)
                }
                if (make_fuzz_t<bool>(fuzz)) {
                    fuzz_paint(fuzz, &paint, depth - 1);
                    FUZZ_ENDPOINT(saveLayerRec.fPaint = &paint;)
                }
                sk_sp<SkImageFilter> imageFilter;
                if (make_fuzz_t<bool>(fuzz)) {
                    imageFilter = make_fuzz_imageFilter(fuzz, depth - 1);
                    FUZZ_ENDPOINT(saveLayerRec.fBackdrop = imageFilter.get();)
                }
                ...
                FUZZ_ENDPOINT(canvas->saveLayer(saveLayerRec);)
                break;
            }
            case 9:
                FUZZ_ENDPOINT(canvas->restore();)
                break;
            case 10: {
                int saveCount;
                fuzz->next(&saveCount);
                FUZZ_ENDPOINT(canvas->restoreToCount(saveCount);)
                break;
            }
            case 11: {
                SkScalar x, y;
                fuzz->next(&x, &y);
                FUZZ_ENDPOINT(canvas->translate(x, y);)
                break;
            }
            case 12: {
                SkScalar x, y;
                fuzz->next(&x, &y);
                FUZZ_ENDPOINT(canvas->scale(x, y);)
                break;
            }
            case 13: {
                SkScalar v;
                fuzz->next(&v);
                FUZZ_ENDPOINT(canvas->rotate(v);)
                break;
            }
            case 14: {
                SkScalar x, y, v;
                fuzz->next(&x, &y, &v);
                FUZZ_ENDPOINT(canvas->rotate(v, x, y);)
                break;
            }
            case 15: {
                SkScalar x, y;
                fuzz->next(&x, &y);
                FUZZ_ENDPOINT(canvas->skew(x, y);)
                break;
            }
            case 16: {
                SkMatrix mat;
                FuzzNiceMatrix(fuzz, &mat);
                FUZZ_ENDPOINT(canvas->concat(mat);)
                break;
            }
            case 17: {
                SkMatrix mat;
                FuzzNiceMatrix(fuzz, &mat);
                FUZZ_ENDPOINT(canvas->setMatrix(mat);)
                break;
            }
            case 18:
                FUZZ_ENDPOINT(canvas->resetMatrix();)
                break;
            case 19: {
                SkRect r;
                int op;
                ...
                FUZZ_ENDPOINT(canvas->clipRect(r, (SkClipOp)op, doAntiAlias);)
                break;
            }
            case 20: {
                SkRRect rr;
                ...
                fuzz->nextRange(&op, 0, 1);
                FUZZ_ENDPOINT(canvas->clipRRect(rr, (SkClipOp)op, doAntiAlias);)
                break;
            }
            case 21: {
                SkPath path;
                ...
                fuzz->next(&doAntiAlias);
                fuzz->nextRange(&op, 0, 1);
                FUZZ_ENDPOINT(canvas->clipPath(path, (SkClipOp)op, doAntiAlias);)
                break;
            }
            case 22: {
                SkRegion region;
                ...
                fuzz->next(&region);
                fuzz->nextRange(&op, 0, 1);
                FUZZ_ENDPOINT(canvas->clipRegion(region, (SkClipOp)op);)
                break;
            }
            case 23:
                fuzz_paint(fuzz, &paint, depth - 1);
                FUZZ_ENDPOINT(canvas->drawPaint(paint);)
                break;
            case 24: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(canvas->drawPoints(pointMode, count, pts, paint);)
                break;
            }
            case 25: {
                fuzz_paint(fuzz, &paint, depth - 1);
                SkRect r;
                fuzz->next(&r);
                FUZZ_ENDPOINT(if (!r.isFinite()) {
                    break;
                })
                FUZZ_ENDPOINT(canvas->drawRect(r, paint);)
                break;
            }
            case 26: {
                fuzz_paint(fuzz, &paint, depth - 1);
                SkRegion region;
                fuzz->next(&region);
                FUZZ_ENDPOINT(canvas->drawRegion(region, paint);)
                break;
            }
            case 27: {
                fuzz_paint(fuzz, &paint, depth - 1);
            ...
            case 29: {
                ...
                SkRRect rr;
                FuzzNiceRRect(fuzz, &rr);
                FUZZ_ENDPOINT(canvas->drawRRect(rr, paint);)
                break;
            }
            case 30: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(if (orr.getBounds().contains(irr.getBounds())) {
            ...
            case 31: {
                ...
                FUZZ_ENDPOINT(canvas->drawArc(r, start, sweep, useCenter, paint);)
                break;
            }
            case 32: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(canvas->drawPath(path, paint);)
                break;
            }
            case 33: {
                ...
                FUZZ_ENDPOINT(canvas->drawImage(img.get(), left, top, SkSamplingOptions(),
            ...
            case 35: {
                ...
                FUZZ_ENDPOINT(canvas->drawImageRect(img.get(), SkRect::Make(src), dst, SkSamplingOptions(),
            ...
            case 37: {
                ...
                fuzz->next(&dst);
                FUZZ_ENDPOINT(canvas->drawImageNine(img.get(), center, dst, SkFilterMode::kNearest,
                                      usePaint ? &paint : nullptr);)
                break;
            }
            case 44: {
                ...
                FUZZ_ENDPOINT(canvas->drawImageLattice(img.get(), lattice, dst, SkFilterMode::kLinear,
                                         usePaint ? &paint : nullptr);)
                break;
            }
            case 45: {
                ...
                fuzz->next(&x, &y);
                SkTDArray<uint8_t> text = make_fuzz_text(fuzz, font, encoding);
                FUZZ_ENDPOINT(canvas->drawSimpleText(text.begin(), SkToSizeT(text.count()), encoding, x, y,
                                       font, paint);)
                break;
            }
            case 46: {
            ...
            case 51: {
                ...
                SkScalar x, y;
                fuzz->next(&x, &y);
                FUZZ_ENDPOINT(canvas->drawTextBlob(blob, x, y, paint);)
                break;
            }
            case 52: {
                SkMatrix matrix;
                ...
                auto pic = make_fuzz_picture(fuzz, depth - 1);
                FUZZ_ENDPOINT(canvas->drawPicture(pic, useMatrix ? &matrix : nullptr,
                                    usePaint ? &paint : nullptr);)
                break;
            }
            case 53: {
                ...
                FUZZ_ENDPOINT(canvas->drawVertices(SkVertices::MakeCopy(vertexMode, vertexCount, vertices,
            ...
            case 54: {
                ...
                fuzz->next(&color);
                FUZZ_ENDPOINT(canvas->drawColor(color, blendMode);)
                break;
            }
            case 55: {
                SkColor4f color;
            ...
            case 56: {
                ...
                SkPoint p0, p1;
                fuzz->next(&p0, &p1);
                FUZZ_ENDPOINT(canvas->drawLine(p0, p1, paint);)
                break;
            }
            case 57: {
                fuzz_paint(fuzz, &paint, depth - 1);
                SkIRect r;
                fuzz->next(&r);
                FUZZ_ENDPOINT(canvas->drawIRect(r, paint);)
                break;
            }
            case 58: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(canvas->drawCircle(center, radius, paint);)
                break;
            }
            case 59: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(canvas->drawArc(oval, startAngle, sweepAngle, useCenter, paint);)
                break;
            }
            case 60: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                FUZZ_ENDPOINT(canvas->drawRoundRect(rect, rx, ry, paint);)
                break;
            }
            case 61: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                SkScalar x, y;
                fuzz->next(&x, &y);
                FUZZ_ENDPOINT(canvas->drawString(str.c_str(), x, y, font, paint);)
                break;
            }
            case 62: {
                fuzz_paint(fuzz, &paint, depth - 1);
                ...
                SkBlendMode mode;
                fuzz->nextEnum(&mode, SkBlendMode::kLastMode);
                FUZZ_ENDPOINT(canvas->drawPatch(cubics, colors, useTexCoords ? texCoords : nullptr
                    , mode, paint);)
                break;
            }
            default:
...
static sk_sp<SkPicture> make_fuzz_picture(Fuzz* fuzz, int depth) {
    SkScalar w, h;
    fuzz->next(&w, &h);
    FUZZ_ENDPOINT(SkPictureRecorder pictureRecorder;
    fuzz_canvas(fuzz, pictureRecorder.beginRecording(w, h), depth - 1);)
    return FUZZ_ENDPOINT(pictureRecorder.finishRecordingAsPicture();)
}
